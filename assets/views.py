...
def run_checks(request):
    ...
    for asset in assets:
        ...
        if now <= asset.service_time <= upcoming:
            ...
            if created:
                logger.info(
                    f"Notification created for service: {asset.name} (ID: {asset.id})"
                )
        ...
        if now <= asset.expiration_time <= upcoming:
            ...
            if created:
                logger.info(
                    f"Notification created for expiration: {asset.name} (ID: {asset.id})"
                )
        ...
        if asset.service_time < now and not asset.is_serviced:
            violation, created = Violation.objects.get_or_create(
                asset=asset, issue=f"Service overdue for asset '{asset.name}'"
            )
            if created:
                logger.warning(
                    f"Violation (service) logged for: {asset.name} (ID: {asset.id})"
                )

        # Violation for expired asset
        if asset.expiration_time < now:
            violation, created = Violation.objects.get_or_create(
                asset=asset, issue=f"Asset '{asset.name}' has expired"
            )
            if created:
                logger.warning(
                    f"Violation (expiration) logged for: {asset.name} (ID: {asset.id})"
                )
